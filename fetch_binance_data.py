...
def fetch_historical_data(symbol="BTCUSDT", interval="1d", months=6):
    ...
    while current_start < end_timestamp:
        current_end = min(
            current_start + (1000 * 24 * 60 * 60 * 1000), end_timestamp
        )  # 1000 gün

        print(
            f"Veri çekiliyor: {datetime.fromtimestamp(current_start / 1000).strftime('%Y-%m-%d')} - {datetime.fromtimestamp(current_end / 1000).strftime('%Y-%m-%d')}"
        )
